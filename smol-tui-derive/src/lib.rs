...
pub fn scene_derive(input: TokenStream) -> TokenStream {
    ...
    let widget_renders = widgets.into_iter().map(|widget| {
        ...
        quote! {
            ::smol_tui::FixedWidget::render(&self.#id,
                &mut frame.subframe::<#x, #y, #w, #h>());
        }
    });
    ...
    let gen = quote! {
        impl ::smol_tui::Scene<#t, #w, #h> for #name {
            fn render(&self, frame: &mut ::smol_tui::FixedFrameAccessor<#t, #w, #h>) {
                #(#widget_renders)*
            }
        }
    };
